...
void RenderingPropertiesWindow::init(){
	...
	std::vector<std::string> light_type = {"Scene light","Head light","Camera light"};
	...
	// this -> new_light_combo_box -> insertItem(1,tr("Head light"));
...
// 	light -> SetLightType(this -> new_light_combo_box -> currentIndex()); // 0: scene , 1: Head light , 2: camera light
...
// 	model_data -> set_actor(lightActor);


// 	renderer->AddViewProp(lightActor);


// 	unsigned int count = this -> wrapped_light_data.count(name);
// 	if(count == 0){
